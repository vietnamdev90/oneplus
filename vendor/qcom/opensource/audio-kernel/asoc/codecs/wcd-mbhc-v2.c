...
/* when disable l det, did not required mech plug detection */
	if (mbhc->mbhc_cfg->enable_usbc_analog) {
#else /* OPLUS_ARCH_EXTENDS */
	if ((mbhc->mbhc_cfg->enable_usbc_analog && mbhc->aatc_dev_np)
...
#if IS_ENABLED(CONFIG_QCOM_FSA4480_I2C) || IS_ENABLED(CONFIG_QCOM_WCD_USBSS_I2C) || IS_ENABLED(CONFIG_OPLUS_TYPEC_SWITCH_I2C)
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		if (IS_ENABLED(CONFIG_QCOM_FSA4480_I2C))
			mbhc->fsa_aatc_dev_np = of_parse_phandle(card->dev->of_node,
							"fsa4480-i2c-handle", 0);
		if (IS_ENABLED(CONFIG_OPLUS_TYPEC_SWITCH_I2C)) {
			mbhc->oplus_aatc_dev_np = of_parse_phandle(card->dev->of_node,
							"oplus-switch-handle", 0);
		}
		if (!mbhc->aatc_dev_np && !mbhc->fsa_aatc_dev_np && !mbhc->oplus_aatc_dev_np) {
#endif /* OPLUS_ARCH_EXTENDS */
...
/* decouple mbhc register from fsa4480 state */
		...
		if (mbhc->oplus_aatc_dev_np) {
			rc = typec_switch_reg_notifier(&mbhc->aatc_dev_nb, mbhc->oplus_aatc_dev_np);
/* decouple mbhc register from typec_switch state */
			if (rc) {
...
#endif
#if IS_ENABLED(CONFIG_OPLUS_TYPEC_SWITCH_I2C)
	if (mbhc->mbhc_cfg->enable_usbc_analog && mbhc->oplus_aatc_dev_np)
		typec_switch_unreg_notifier(&mbhc->aatc_dev_nb, mbhc->oplus_aatc_dev_np);
#endif
#endif /* OPLUS_ARCH_EXTENDS */
...
#ifdef OPLUS_ARCH_EXTENDS
/* Add for L_DET gpio level shift */
	mbhc->det_gpio_pinctrl = devm_pinctrl_get(card->dev);
	if (IS_ERR_OR_NULL(mbhc->det_gpio_pinctrl)) {
		pr_info("%s: det_gpio_pinctrl is null\n", __func__);
	} else {
		mbhc->det_gpio_active = pinctrl_lookup_state(mbhc->det_gpio_pinctrl , "det_gpio_active");
		if (IS_ERR_OR_NULL(mbhc->det_gpio_active)) {
			pr_info("%s: det_gpio_active not provide\n", __func__);
		} else {
			ret = pinctrl_select_state(mbhc->det_gpio_pinctrl, mbhc->det_gpio_active);
			if (ret) {
				pr_err("%s: fail to set pin state for det_gpio_active\n", __func__);
			} else {
				pr_info("%s: success to set pin state for det_gpio_active\n", __func__);
			}
		}
	}
#endif /* OPLUS_ARCH_EXTENDS */
