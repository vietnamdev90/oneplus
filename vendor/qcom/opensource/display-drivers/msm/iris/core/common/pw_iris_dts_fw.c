...
static int iris_fw_count_elems_of_size(const struct device_node *np,
				const char *propname, int elem_size)
{
	...
	if (p_node->len % elem_size == 0)
		return p_node->len / elem_size;
...
static int iris_fw_count_u8_elems(const struct device_node *np,
				const char *propname)
{
	return iris_fw_count_elems_of_size(np, propname, sizeof(u8));
}

static int iris_fw_count_u32_elems(const struct device_node *np,
				const char *propname)
{
	return iris_fw_count_elems_of_size(np, propname, sizeof(u32));
}
...
static struct iris_dts_ops g_fw_dts_ops = {
	...
	.count_u32_elems = iris_fw_count_u32_elems,
};
...
int iris_parse_dts_ctx(const uint8_t *fw_name)
{
	...
	if (!ret)
		IRIS_LOGI("%s(%d), load name: %s, size: %zu.",
