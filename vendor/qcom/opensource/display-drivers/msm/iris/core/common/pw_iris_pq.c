...
u8 iris_sdr2hdr_mode;
static u32 last_filter_level;
bool shadow_iris_HDR10_YCoCg;
bool iris_debug_cap;
...
int iris_get_hdr_enable(void)
{
	...
	if (pcfg->pw_chip_func_ops.iris_get_hdr_enable_)
		return pcfg->pw_chip_func_ops.iris_get_hdr_enable_();
	else
		return 0;
}
...
void iris_quality_setting_off(void)
{
	...
	if (pcfg->pw_chip_func_ops.iris_quality_setting_off_)
		pcfg->pw_chip_func_ops.iris_quality_setting_off_();
...
bool iris_crst_coef_check(const u8 *fw_data, size_t fw_size, u32 crstk_coef_group)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		crstk_coef_group = CRSTK_COEF_GROUP_I7;
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		crstk_coef_group = CRSTK_COEF_GROUP_I7P;
	else if (pcfg->iris_chip_type == CHIP_IRIS8)
		crstk_coef_group = CRSTK_COEF_GROUP_I8;
	...
	if (iris_crstk_coef_buf == NULL) {
		...
		IRIS_LOGI("kzalloc iris_crstk_coef_buf:%p", iris_crstk_coef_buf);
	}
...
{
	struct iris_cfg *pcfg = iris_get_cfg();
	if (pcfg->pw_chip_func_ops.iris_pq_parameter_init_)
		pcfg->pw_chip_func_ops.iris_pq_parameter_init_();
}
...
static void iris_cm_ratio_set(void)
{
	...
	if (pcfg->pw_chip_func_ops.iris_cm_ratio_set_)
		pcfg->pw_chip_func_ops.iris_cm_ratio_set_();
}
...
void iris_cm_color_gamut_set(u32 level, bool bcommit)
{
	...
	if (pcfg->pw_chip_func_ops.iris_cm_color_gamut_set_)
		pcfg->pw_chip_func_ops.iris_cm_color_gamut_set_(level, bcommit);
}
...
void iris_dpp_gamma_set(void)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS5) {
		if (pcfg->pw_chip_func_ops.iris_dpp_gamma_set_i5_)
			return pcfg->pw_chip_func_ops.iris_dpp_gamma_set_i5_();
	}
...
void iris_dpp_precsc_enable(u32 enable, bool bcommit)
{
	...
	if (pcfg->pw_chip_func_ops.iris_dpp_precsc_enable_)
		pcfg->pw_chip_func_ops.iris_dpp_precsc_enable_(enable, bcommit);
}
...
void iris_lux_set(u32 level, bool update)
{
	...
	if (pcfg->pw_chip_func_ops.iris_lux_set_)
		pcfg->pw_chip_func_ops.iris_lux_set_(level, update);
}
...
void iris_al_enable(bool enable)
{
	...
	if (pcfg->pw_chip_func_ops.iris_al_enable_)
		pcfg->pw_chip_func_ops.iris_al_enable_(enable);
}
...
void iris_pwil_dport_disable(bool enable, u32 value)
{
	...
	if (pcfg->pw_chip_func_ops.iris_pwil_dport_disable_)
		pcfg->pw_chip_func_ops.iris_pwil_dport_disable_(enable, value);
}
...
int iris_kickoff(bool is_secondary)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	if (pcfg->fod_pending)
		iris_post_fod(is_secondary);

	if (pcfg->memc_info.vfr_en && (pcfg->iris_chip_type == CHIP_IRIS7 ||
				       pcfg->iris_chip_type == CHIP_IRIS8))
		iris_memc_vfr_video_update_monitor(pcfg, is_secondary);
	if (pcfg->dynamic_vfr && (pcfg->iris_chip_type == CHIP_IRIS5))
		iris_memc_vfr_video_update_monitor(pcfg, is_secondary);

	if (is_secondary || pcfg->valid < PARAM_PARSED)
		return 0;
		...
		if (ip == DBC_LUT) {
			for (i = 0; i < pq_ipopt_val->opt_cnt; i++) {
				...
				if (old_opt < CABC_DLV_OFF && (old_opt & 0x7f) != 0) {
					...
					cnt = iris_init_update_ipopt_t(ip,
							old_opt, opt_id, chain);
					return cnt;
				}
			}
		}
		...
		size_t count, loff_t *ppos)
{
	...
	if (pcfg->pw_chip_func_ops.iris_configure_)
		pcfg->pw_chip_func_ops.iris_configure_(IRIS_DSI_PRIMARY, type, value);

	for (i = 0; i < len; i++) {
		if (pcfg->pw_chip_func_ops.iris_configure_get_)
			pcfg->pw_chip_func_ops.iris_configure_get_(IRIS_DSI_PRIMARY, arr[i].type, 1, &cfg_val);
...
int iris_dbgfs_pq_init(void)
{
	...
	if (debugfs_create_file("iris_pq_config", 0644, pcfg->dbg_root, NULL,
...
int iris_update_backlight(u32 bl_lvl)
{
	...
	struct iris_cmd_desc backlight_cmd = {
		{
			.channel = 0,
			.type = MIPI_DSI_DCS_LONG_WRITE,
			.flags = MIPI_DSI_MSG_LASTCOMMAND,
			.tx_len = sizeof(led_pwm1),
			.tx_buf = led_pwm1,
			.rx_len = 0,
			.rx_buf = NULL,
			},
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7) {
		...
		iris_setting.quality_cur.ai_backlight = ((u32)pcfg->panel_nits*bl_lvl)/bl_max_level;
		if ((iris_setting.quality_cur.ai_auto_en == AI_BACKLIGHT_ENABLE
			...
			pcfg->pw_chip_func_ops.iris_hdr_ai_input_bl_)
			pcfg->pw_chip_func_ops.iris_hdr_ai_input_bl_(iris_setting.quality_cur.ai_backlight, true);
	}
...
void iris_dom_set(int mode)
{
	...
	if (pcfg->pw_chip_func_ops.iris_dom_set_)
		pcfg->pw_chip_func_ops.iris_dom_set_(mode);
}
...
void iris_csc2_para_set(uint32_t *values)
{
	...
	if (pcfg->pw_chip_func_ops.iris_csc2_para_set_)
		pcfg->pw_chip_func_ops.iris_csc2_para_set_(values);
}
...
void iris_pwil_dpp_en(bool dpp_en)
{
	...
	if (pcfg->pw_chip_func_ops.iris_pwil_dpp_en_)
		pcfg->pw_chip_func_ops.iris_pwil_dpp_en_(dpp_en);
}

void iris_dpp_en(bool dpp_en)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	if (pcfg->iris_chip_type == CHIP_IRIS8 && pcfg->pw_chip_func_ops.iris_dpp_en_)
		pcfg->pw_chip_func_ops.iris_dpp_en_(dpp_en);
}
...
int iris_EDR_backlight_ctrl(u32 hdr_nit, u32 ratio_panel)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7) {
		if (pcfg->pw_chip_func_ops.iris_EDR_backlight_ctrl_i7_)
			rc = pcfg->pw_chip_func_ops.iris_EDR_backlight_ctrl_i7_(hdr_nit, ratio_panel);
	}
	else
		rc = -EINVAL;
