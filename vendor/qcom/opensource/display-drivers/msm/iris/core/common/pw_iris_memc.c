...
{
	if (!memc_func.iris_demo_wnd_set)
		return;

	memc_func.iris_demo_wnd_set();
}
...
static void _iris_demo_wnd_border_color_calc(void)
{
	...
	if (chip == CHIP_IRIS7 || chip == CHIP_IRIS7P)
		pcfg->demo_color = (y << 16) | (v << 8) | u;
	else if (chip == CHIP_IRIS8)
		pcfg->demo_color = (v << 16) | (u << 8) | y;
}
...
void iris_memc_func_init(void)
{
	...
	if (pcfg->pw_chip_func_ops.iris_memc_func_init_)
		pcfg->pw_chip_func_ops.iris_memc_func_init_(&memc_func);
}

void iris_register_osd_irq(void *disp)
{
	if (!memc_func.register_osd_irq) {
		IRIS_LOGE("%s, register_osd_irq not init!", __func__);
		return;
	}
...
void iris_update_panel_ap_te(void *handle, u32 new_te)
{
	...
	memc_func.update_panel_ap_te(handle, new_te);
}
...
bool iris_osd_drm_autorefresh_enabled(bool is_secondary)
{
	...
	rc = memc_func.is_display1_autorefresh_enabled(is_secondary);
...
{
	if (!memc_func.get_drm_property)
		return 0;

	return memc_func.get_drm_property(id);
}
...
void iris_enable_memc(bool is_secondary)
{
	...
	memc_func.enable_memc(is_secondary);
}
...
{
	if (!memc_func.parse_memc_param)
		return;

	memc_func.parse_memc_param();
}
...
void iris_memc_vfr_video_update_monitor(struct iris_cfg *pcfg, bool is_secondary)
{
	...
	memc_func.memc_vfr_video_update_monitor(pcfg, is_secondary);
}
...
{
	if (!memc_func.not_allow_off_primary)
		return false;

	return memc_func.not_allow_off_primary();
}

bool iris_not_allow_off_secondary(void)
{
	if (!memc_func.not_allow_off_secondary)
		return false;

	return memc_func.not_allow_off_secondary();
}

int iris_blending_enable(bool enable)
{
	if (!memc_func.blending_enable)
		return 0;

	return memc_func.blending_enable(enable);
}
u32 iris_disable_mipi1_autorefresh_get(void)
{
	if (!memc_func.disable_mipi1_autorefresh)
		return 0;
