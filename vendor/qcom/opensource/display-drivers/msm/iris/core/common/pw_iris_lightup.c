...
static u32 iris_generate_frc_cmd_list = 1;
...
bool iris_check_seq_ipopt(u32 ip, u32 opt)
{
	...
	struct iris_ctrl_opt *pseq_opt = pseq->ctrl_opt;

	for (i = 0; i < pseq->cnt; i++) {
		if (ip == pseq_opt[i].ip && opt == pseq_opt[i].opt_id)
			return true;
	}
	return false;
}
		...
		struct iris_ip_index *pip_index)
{
	...
	if (pip_index[ip].opt[i].link_state == 0xff
		...
		pip_index[ip].opt[i].link_state =
			link_state ? IRIS_CMD_SET_STATE_LP : IRIS_CMD_SET_STATE_HS;
	}
		...
		const struct iris_cmd_header *src, int index, const int pkt_cnt)
{
	...
	dest->last_pkt = 0; //TODO: iris5: (pcfg->add_last_flag) ? 1: 0;
		...
		struct iris_cfg *pcfg)
{
	...
	if (rc) {
		IRIS_LOGW("%s(), failed to %s, return: %d",
			__func__, key, rc);
		...
		struct iris_cfg *pcfg)
{
	...
	/* Bit(0) identify FPGA, so value 2 also is FPGA */
	if (pcfg->platform_type == 2)
		pcfg->platform_type = 0;
		...
		struct device_node *np, struct iris_cfg *pcfg)
{
	...
	if (data) {
		if (!strcmp(data, "single"))
			pcfg->lut_mode = IRIS_SINGLE_MODE;
		...
		struct iris_cfg *pcfg)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS5 || pcfg->iris_chip_type == CHIP_IRIS7) {
		pcfg->ovs_delay = payload[13] + 1;
		pcfg->ovs_delay_frc = payload[27] + 1;
	} else {
		...
		struct iris_cfg *pcfg)
{
	...
	if (!payload) {
		IRIS_LOGW("%s(), can not get eco property in pwil setting", __func__);
...
{
	pctrl_seq->ctrl_opt = kvmalloc(seq_cnt * sizeof(struct iris_ctrl_opt), GFP_KERNEL);
...
static void _iris_parse_default_aux_size(struct iris_cfg *pcfg)
{
	...
	if (!payload) {
		IRIS_LOGW("%s(), failed to find IRIS_IP_DSC_DEC_AUX", __func__);
		return;
	}
...
static void _iris_parse_ap_panel_te(struct device_node *np)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();
	...
	IRIS_LOGI("pcfg->panel_te: %d", pcfg->panel_te);
	pcfg->ap_te = 60;
	p_dts_ops->read_u32(np, "pxlw,ap-te", &(pcfg->ap_te));
	IRIS_LOGI("pcfg->ap_te: %d", pcfg->ap_te);

	pcfg->rfb_buf_num = 1;
	p_dts_ops->read_u32(np, "pxlw,rfb-buf-num", &(pcfg->rfb_buf_num));
	pcfg->rfb_bypass_dsc = p_dts_ops->read_bool(np, "pxlw,rfb-bypass-dsc");
	IRIS_LOGI("rfb_buf_num: %d, rfb_bypass_dsc: %d", pcfg->rfb_buf_num, pcfg->rfb_bypass_dsc);
}

void _iris_parse_frc_dec_initial_delay(struct device_node *np)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	int ret = 0;
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();
	...
	pcfg->frc_dsc_init_delay = 0;
	ret = p_dts_ops->read_u32(np, "pxlw,frc-dsc-init-delay", &pcfg->frc_dsc_init_delay);
	if (ret != 0) {
...
static void _iris_parse_default_mode(
		struct device_node *np, struct iris_cfg *pcfg)
{
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();
	...
	pcfg->default_rfb = p_dts_ops->read_bool(np,
			"pxlw,iris-default-rfb");
	IRIS_LOGI("%s(), iris default rfb: %s",
			__func__,
			pcfg->default_rfb ? "true" : "false");

	if (iris_is_graphic_memc_supported())
		pcfg->pq_switch = 2;
}

static int  _pw_iris_parse_subnode(void *node)
{
	int32_t rc = -EINVAL;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	//iris_parse_memc_param0();
	_iris_parse_ap_panel_te(lightup_node);

	_iris_parse_frc_dec_initial_delay(lightup_node);
#if 0
	...
	_iris_parse_default_mode(lightup_node, pcfg);
...
int pw_iris_parse_param(struct device_node *lightup_node)
{
	...
	ret = _pw_iris_parse_subnode(lightup_node);
...
void iris_print_desc_cmds(struct iris_cmd_desc *p, int cmd_cnt, int state)
{
	int i = 0;
	int j = 0;
	int msg_len = 0;
	...
	IRIS_LOGI("%s(), cmd len: %d, state: %s", __func__, cmd_cnt,
			(state == IRIS_CMD_SET_STATE_HS) ? "high speed" : "low power");
...
static void _iris_print_total_cmds(struct iris_cmd_desc *cmds, int cmd_cnt)
{
	int i = 0;
	int j = 0;
	int value_count = 0;
...
static int32_t _iris_i2c_send_ocp_cmds(struct iris_cmd_comp *pcmd_comp)
{
	int ret = 0;
	int i = 0;
...
static int32_t _iris_i2c_s_send_ocp_cmds(struct iris_cmd_comp *pcmd_comp)
{
	...
	for (i = 0; i < pcmd_comp->cnt; i++) {
		...
		if (pcfg->iris_i2c_conver_ocp_write)
			ret = pcfg->iris_i2c_conver_ocp_write(payload[0], payload, len, is_burst);
		else
			IRIS_LOGE("%s(), error, not support", __func__);

		if (ret) {
			IRIS_LOGE("%s(), error, ret = %d", __func__, ret);
			return ret;
		}
	}
	return 0;
...
static int32_t _iris_dsi_single_addr_send(struct iris_cmd_comp *pcmd_comp)
{
	int32_t ret, i;
	uint32_t wait = 0;
	uint32_t trans_len, cont_pkt_len;
	struct iris_cmd_desc *cmd = NULL;
	struct iris_cmd_desc *cmd_cur = NULL;
	struct iris_cmd_desc *cmd_next = NULL;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	if ((trans_len == 0) || (trans_len % split_cmd_len == 0)) {
		/*fill last_command*/
		_iris_set_pkt_last(pcmd_comp->cmd, pcmd_comp->cnt,
			DSI_CMD_ONE_LAST_FOR_MULT_IPOPT);
	} else {
		_iris_set_pkt_last(pcmd_comp->cmd, pcmd_comp->cnt, trans_len);
		cont_pkt_len = 0;
	...
	if (IRIS_IF_LOGD()) {
		for (i = 0; i < pcmd_comp->cnt; i++) {
			cmd = pcmd_comp->cmd + i;
			IRIS_LOGD("i = %d, tx_len = %zu, last = %d",
	...
	ret = pcfg->lightup_ops.transfer(pcmd_comp->cmd,
...
static void __iris_mult_addr_pad(uint8_t **p, uint32_t *poff, uint32_t left_len)
{
	...
	if (pcfg->pw_chip_func_ops.iris_mult_addr_pad_)
		pcfg->pw_chip_func_ops.iris_mult_addr_pad_(p, poff, left_len);
}

static void __iris_mult_addr_sw_split(uint8_t **p, uint32_t *poff,
	uint32_t left_len, struct iris_cmd_desc *pdesc)
{
	uint8_t *ptmp = NULL;
...
static int32_t _iris_dsi_mult_addr_send(struct iris_cmd_comp *pcmd_comp)
{
	int32_t ret = 0;
	...
	pdesc = vzalloc(trans_num * sizeof(struct iris_cmd_desc));
	if (!pdesc) {
	...
	ret = pcfg->lightup_ops.transfer(pdesc,
...
static int32_t _iris_dsi_send_ocp_cmds(struct iris_cmd_comp *pcmd_comp)
{
	...
	if (pcfg->lightup_ops.change_header)
		pcfg->lightup_ops.change_header(pcmd_comp);
	switch (dsi_trans_mode) {
	case DSI_EMBEDDED_NO_MA_MODE:
		ret = _iris_dsi_single_addr_send(pcmd_comp);
		break;
	case DSI_EMBEDDED_MA_MODE:
	case DSI_NON_EMBEDDED_MODE:
		ret = _iris_dsi_mult_addr_send(pcmd_comp);
		break;
	default:
...
static void _iris_send_panel_cmd(struct iris_cmd_set *cmds)
{
	if (!cmds || !cmds->count) {
...
int32_t iris_send_ipopt_cmds(int32_t ip, int32_t opt_id)
{
	...
	cmd = NULL;
	cmd = kvzalloc(cmd_comp.cnt * sizeof(struct iris_cmd_desc), GFP_KERNEL);
	if (!cmd) {
		IRIS_LOGE("%s(), failed to alloc desc cmd", __func__);
		return -ENOMEM;
	}
	memcpy(cmd, cmd_comp.cmd, cmd_comp.cnt * sizeof(struct iris_cmd_desc));
	cmd_comp.cmd = cmd;
...
static void _iris_send_spec_lut_i5(uint8_t lut_table, uint8_t lut_idx)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	if (pcfg->pw_chip_func_ops.iris_send_lut_i5_) {
		if (lut_table == DBC_LUT && lut_idx < CABC_DLV_OFF)
			pcfg->pw_chip_func_ops.iris_send_lut_i5_(lut_table, lut_idx, 1);

		pcfg->pw_chip_func_ops.iris_send_lut_i5_(lut_table, lut_idx, 0);
	}
}
...
static void _iris_send_spec_lut(uint8_t lut_table, uint8_t lut_idx)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS5)
		_iris_send_spec_lut_i5(lut_table, lut_idx);
	else {
		if (pcfg->pw_chip_func_ops.iris_send_lut_)
			pcfg->pw_chip_func_ops.iris_send_lut_(lut_table, lut_idx);
	}
}
...
static void _iris_send_new_lut(uint8_t lut_table, uint8_t lut_idx)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS5) {
		if (lut_table == DBC_LUT && pcfg->pw_chip_func_ops.iris_get_dbc_lut_index_)
			dbc_lut_index = pcfg->pw_chip_func_ops.iris_get_dbc_lut_index_();
		if (pcfg->pw_chip_func_ops.iris_send_lut_i5_)
			pcfg->pw_chip_func_ops.iris_send_lut_i5_(lut_table, lut_idx, dbc_lut_index);
	} else {
		if (pcfg->pw_chip_func_ops.iris_send_lut_)
			pcfg->pw_chip_func_ops.iris_send_lut_(lut_table, lut_idx);
	}
...
void _iris_send_lightup_pkt(void)
{
	...
	IRIS_ATRACE_BEGIN("_iris_send_lightup_pkt");
	iris_send_assembled_pkt(pseq->ctrl_opt, pseq->cnt);
	IRIS_ATRACE_END("_iris_send_lightup_pkt");
}
...
int iris_init_update_ipopt_t(uint8_t ip, uint8_t opt_old, uint8_t opt_new, uint8_t chain)
{
	...
	if (index >= max_cnt) {
		IRIS_LOGE("%s(), no empty space to install i_p: %#x, opt old: %#x, opt new: %#x max_cnt %u",
				__func__, ip, opt_old, opt_new, max_cnt);
		return -EINVAL;
	}
	...
	/* Filter out duplicated item. */
	popt = pcfg->pq_update_cmd.update_ipopt_array;
	for (i = 0; i < index; i++) {
		if (popt[i].ip == ip && popt[i].opt_old == opt_old &&
				popt[i].opt_new == opt_new) {
			if (chain == 0)
				popt[index - 1].chain = chain;
			return index;
		}
	}
...
int _iris_read_chip_id(void)
{
	...
	if (pcfg->chip_value[0] != 0)
		pcfg->chip_value[1] = iris_ocp_read(pcfg->chip_value[0], IRIS_CMD_SET_STATE_HS);

	// FIXME: if chip version is set by sw, skip hw read chip id.
	if (pcfg->chip_ver == IRIS3_CHIP_VERSION)
		pcfg->chip_id = (iris_ocp_read(sys_pll_ro_status, IRIS_CMD_SET_STATE_HS)) & 0xFF;
	else
		pcfg->chip_id = 0;
...
static void _iris_clean_status(void)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS5)
		if (pcfg->pw_chip_func_ops.iris_clean_frc_status_)
			pcfg->pw_chip_func_ops.iris_clean_frc_status_(pcfg);
}
...
{
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 values[2] = {(pcfg->iris_chip_type == CHIP_IRIS8) ? 0xF00001C4 : 0xF00000C8, 0x1};
	struct iris_ctrl_opt ctrl_opt;

	IRIS_LOGI("%s(%d): load mcu, mcu_sw_reset:%d (0x%x)", __func__, __LINE__, values[1], values[0]);
...
static void _iris_pre_timing_config(void)
{
	...
	//sys pll
	iris_update_ip_opt(IRIS_IP_SYS, use_2nd_timing ? 0xA1 : 0xA0, 0x1);
	//dtg
	iris_update_ip_opt(IRIS_IP_DTG, use_2nd_timing ? 0x1 : 0x0, 0x1);
	//mipi tx
	iris_update_ip_opt(IRIS_IP_TX, use_2nd_timing ? 0x4 : 0x0, 0x1);
	//mipi abp
	len = iris_update_ip_opt(IRIS_IP_RX, use_2nd_timing ? 0xE1 : 0xE0, 0x0);
...
static void _iris_pre_rx_cmd(void)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS8)
		iris_send_ipopt_cmds(IRIS_IP_RX, pcfg->id_rx_init);
}
void _iris_pre_lightup_i5(void)
{
	...
	iris_pq_parameter_init();
	iris_frc_timing_setting_update();//iris_frc_parameter_init();
	_iris_clean_status();
}


void _iris_pre_lightup(void)  //TODO: iris5 _iris_pre_lightup_i5(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
...
void _iris_read_power_mode(void)
{
	...
	struct iris_cmd_desc cmds = {
		{
			...
			.tx_len = sizeof(get_power_mode),
			.tx_buf = get_power_mode,
			.rx_len = 1,
			.rx_buf =  read_cmd_rbuf,
		}, 1, 0};
	...
	if (pcfg->iris_set_msg_flags)
		pcfg->iris_set_msg_flags(&cmds, READ_FLAG);
...
void iris_update_2nd_active_timing(struct iris_mode_info *timing)
{
	...
	if (pcfg == NULL)
		return;
	if (pcfg->iris_chip_type == CHIP_IRIS7 ||
		pcfg->iris_chip_type == CHIP_IRIS5) {
		if (pcfg->ap_mipi1_power_st) {
			pcfg->frc_setting.hres_2nd = timing->h_active;
			pcfg->frc_setting.vres_2nd = timing->v_active;
			pcfg->frc_setting.refresh_rate_2nd = timing->refresh_rate;
			pcfg->frc_setting.dsc_2nd = timing->dsc_enabled;
...
static void _iris_send_pq_cmds(struct iris_update_ipopt *popt, int ipopt_cnt, uint8_t path)
{
	...
	cmd_comp.link_state = IRIS_CMD_SET_STATE_HS;
...
void iris_update_pq_opt(uint8_t path, bool bcommit)
{
	...
	if (!popt || !ipopt_cnt) {
		IRIS_LOGE("%s(), invalid popt or ipopt_cnt %d", __func__, ipopt_cnt);
		return;
	}
...
void iris_end_last_opt(void)
{
	...
	if (index != 0) {
		if (popt[index - 1].chain == 0)
			IRIS_LOGI("%s i_p: %#x opt: %#x chain has been set 0",
				__func__, popt[index - 1].ip, popt[index - 1].opt_new);
		else
			popt[index - 1].chain = 0;
	}
}
static struct iris_cmd_desc *_iris_get_desc_from_ipopt(uint8_t ip, uint8_t opt_id, int32_t pos)
{
	...
	if (pos < 2) {
		pos = 2;	// to access 0xFFFFFFF4 type
	}
		...
		uint8_t opt_id, int32_t pos, uint32_t type)
{
	...
	if (popt == NULL) {
		IRIS_LOGD("%s(), can't find popt for i_p: 0x%02x, opt: 0x%02x, type: %u",
		...
		uint8_t ip, uint8_t opt_id, int32_t pos)
{
	...
	*/
	...
	if (pos < 2)
		...
		payload_offset = (pos * 4 - IRIS_OCP_HEADER_ADDR_LEN) % pcfg->split_pkt_size + IRIS_OCP_HEADER_ADDR_LEN;

	return (uint32_t *)((uint8_t *)pdesc->msg.tx_buf + payload_offset);
}
...
uint32_t iris_get_ipopt_payload_len(uint8_t ip, uint8_t opt_id)
{
	...
	struct iris_cfg *pcfg = NULL;
	struct iris_cmd_desc *pdesc = NULL;
	uint32_t total_payload_len = IRIS_OCP_HEADER_ADDR_LEN;
	uint32_t i = 0;
	...
	for (i = 0; i < popt->cmd_cnt; i++) {
		pdesc = popt->cmd + i;
		if (!pdesc) {
			IRIS_LOGE("%s(), can't find desc for i_p: 0x%02x, opt: 0x%02x",
				__func__, ip, opt_id);
			return 0;
		}
		total_payload_len += pdesc->msg.tx_len - IRIS_OCP_HEADER_ADDR_LEN;
	}
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%hhx %hhx %x %x", &ip, &opt_id, &pos, &value) != 4)
		return -EINVAL;
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%hhx %hhx %d", &ip, &opt_id, &pos) != 3) {
		if (sscanf(buf, "%hhx %hhx", &ip, &opt_id) == 2) {
			struct iris_ip_opt *popt = NULL;
			int i = 0;
...
static void _iris_fw_create_name(const char *display_name, char *name)
{
	u8 *np = name;
	int len = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		len = sprintf(np, "%s", "pxlw_i7_");
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		len = sprintf(np, "%s", "pxlw_i7p_");
	else if (pcfg->iris_chip_type == CHIP_IRIS8)
		len = sprintf(np, "%s", "pxlw_i8_");
	else if (pcfg->iris_chip_type == CHIP_IRIS5)
		len = sprintf(np, "%s", "pxlw_i5_");

	np += len;
	len = sprintf(np, "%s.fw", display_name);
...
int _iris_fw_parse_dts(const char *display_name)
{
	...
	if (iris_get_cfg()->cmd_param_from_fw) {
		...
		_iris_fw_create_name(display_name, fw_name);
...
static void _iris_dev_remove(struct platform_device *pdev)
{
...
void iris_memc_chain_prepare(void)
{
	...
	if (!pcfg->memc_chain_en)
		return;

	pcfg->memc_chain = true;
}
...
void iris_memc_chain_process(void)
{
	...
	for (i = 0; i < ipopt_cnt; i++) {
		if (popt[i].ip == 0xFF)
			IRIS_LOGW("%s popt[%d] i_p is 0xFF", __func__, i);
		IRIS_LOGD("%s popt[%d].chain: %d, i_p: %d",
			__func__, i, popt[i].chain, popt[i].ip);
		if (i < ipopt_cnt - 1)
			popt[i].chain = 1;
		else
			popt[i].chain = 0;
