...
static int _iris_configure(u32 display, u32 type, u32 value)
{
	...
	case IRIS_DYNAMIC_POWER_CTRL:
		if (value & 0x01) {
			IRIS_LOGI(" [%s, %d] open psr_mif osd first addr eco.", __func__, __LINE__);
			iris_dynamic_power_set(value & 0x01, false);
		} else {
			IRIS_LOGI(" [%s, %d] close psr_mif osd first addr eco.", __func__, __LINE__);
			iris_dynamic_power_set(value & 0x01, false);
		}
		break;
	case IRIS_DBP_MODE:
	...
	case IRIS_ANALOG_BYPASS_MODE:
		...
		if (iris_abyp_switch_proc(value) != 0)
			goto error;
		break;
	...
	case IRIS_DBG_TIMING_SWITCH_LEVEL:
		iris_set_tm_sw_dbg_param(1, &value);
		break;
	case IRIS_WAIT_VSYNC:
		if (pcfg->lightup_ops.wait_vsync)
			return pcfg->lightup_ops.wait_vsync();
		break;
	...
	case IRIS_PT_SR_LUT_SET:
	{
		...
		if (pcfg->abyp_ctrl.abypass_mode == ANALOG_BYPASS_MODE)
			need_lock = false;
		if (need_lock && pcfg->lightup_ops.acquire_panel_lock)
			pcfg->lightup_ops.acquire_panel_lock();
		iris_scl_update_model(1, &value);
		if (need_lock && pcfg->lightup_ops.release_panel_lock)
...
{
	int ret = -1;
	int level, delay, wait_vsync_count;
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_setting_info *iris_setting = iris_get_setting();
	struct quality_setting *pqlt_cur_setting = &iris_setting->quality_cur;
	//uint8_t i = 0;
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_SET2:
		if (adb_type == 0) {
			...
			iris_ocp_write_vals(values[0], values[1], count-2, values+2, IRIS_CMD_SET_STATE_HS);
		} else {
	...
	case IRIS_DBG_SEND_PACKAGE:
		...
		break;
	case IRIS_DBG_TIMING_SWITCH_LEVEL:
		iris_set_tm_sw_dbg_param(count, values);
		break;
	...
	case IRIS_BRIGHTNESS_V2_INFO:
		if (count < 2) {
			IRIS_LOGE("%s(), unrecognized size %u for IRIS_BACKLIGHT_V2_INFO", __func__, count);
			return -EPERM;
		}
		level = values[0];
		delay = values[1];
		wait_vsync_count = values[2];
		iris_set_brightness_v2(level, delay, wait_vsync_count);
		break;
	case IRIS_SPECIAL_MODE_SWITCH:
		iris_set_special_mode(count, values);
		break;
	default:
...
int iris_configure_get_i7p(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		...
		if (adb_type == 0) {
			pcfg->lightup_ops.acquire_panel_lock();
			*values = iris_ocp_read(*values, IRIS_CMD_SET_STATE_HS);
			pcfg->lightup_ops.release_panel_lock();
		} else if (adb_type == 1) {
			reg_addr = *values;
			if (pcfg->iris_i2c_read) {
	...
	case IRIS_DUAL2SINGLE_ST:
		*values = 0;
		break;
	case IRIS_WORK_MODE:
		*values = IRIS_CMD_MODE;
		if (pcfg->get_panel_mode)
			*values = pcfg->get_panel_mode();
		break;
	case IRIS_PANEL_TE:
		*values = 0;
		break;
	...
	case IRIS_MIPI_RX_VALIDATE:
		pcfg->lightup_ops.acquire_panel_lock();
		reg_val = iris_ocp_read(0xf1a00004, IRIS_CMD_SET_STATE_HS);
		pcfg->lightup_ops.release_panel_lock();
