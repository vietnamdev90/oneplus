...
static int iris_debug_dtg = 3;
static int iris_debug_n2m_in_frc_only = 2;
...
void iris_set_pwil_mode_i7p(u8 mode, int state, bool commit)
{
	...
	struct iris_cmd_desc iris_pwil_mode_cmd = {
		{
			.channel = 0,
			.type = MIPI_DSI_GENERIC_SHORT_WRITE_2_PARAM,
			.flags = 0,
			.tx_len = sizeof(pwil_mode),
			.tx_buf = pwil_mode,
		}, 1, 0};
	...
	if (pcfg && pcfg->iris_memc_ops.iris_memc_get_main_panel_dsc_en_info)
	...
	if (commit && pcfg->lightup_ops.send_pwil_cmd)
		pcfg->lightup_ops.send_pwil_cmd(&panel_cmds, IRIS_RX0_ADDR + RX0_VDO_META, meta);
...
void iris_input_frame_cnt_record_i7p(void)
{
	...
	if (!dump->trigger)
		dump->rx_frame_cnt0 = iris_ocp_read(IRIS_RX0_ADDR + DSI_DCS_VDO_ST1, IRIS_CMD_SET_STATE_HS);
}
...
void iris_frc_mif_reg_set_i7p(void)
{
	...
	if (pcfg->rx_mode == IRIS_CMD_MODE && pcfg->tx_mode == IRIS_VIDEO_MODE)
	...
	if (iris_low_latency_mode_get_i7p() == ULTRA_LT_MODE ||
		iris_low_latency_mode_get_i7p() == LT_MODE_DB) {
		/* two buffer mode, disable REP_FRM_DET_EN */
		if (!iris_three_buffer_low_latency && iris_low_latency_mode_get_i7p() == ULTRA_LT_MODE)
	...
	else if (pcfg->rx_mode == IRIS_CMD_MODE && pcfg->tx_mode == IRIS_VIDEO_MODE)
		val_frcc_enable_1 |= 0x200;	// enable FI_MIF_LOCK_EN
	if (iris_low_latency_mode_get_i7p() == ULTRA_LT_MODE) {
...
static bool iris_fallback_allow(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	bool isValid = false;
...
void iris_gmd_reg_set_i7p(void)
{
	...
	if (pcfg->memc_info.memc_mode == MEMC_SINGLE_GAME_ENABLE && iris_fallback_allow()) {
...
void iris_mvc_reg_set_i7p(void)
{
	...
	} else if (mode == ULTRA_LT_MODE || mode == LT_MODE_DB) {
		iris_frc_reg_add_i7p(IRIS_MVC_ADDR + MVC_POSTFILT_0, 0x67b444c1, 0);
		iris_frc_reg_add_i7p(IRIS_MVC_ADDR + MVC_KFBOSD_0, 0x12429f43, 0);
...
void iris_fi_reg_set_i7p(void)
{
	...
	if (iris_memc_dsc_info(DSC_PPS_SET0)->chroma_format == IRIS_CHROMA_444 || debug_raw_cmd_overwrite) {
...
static bool iris_mcu_is_idle_i7p(void)
{
	...
	if (!debug_disable_mcu_check) {
		mcu_sw_reset = iris_ocp_read(IRIS_SYS_ADDR + MCU_SW_RESET, IRIS_CMD_SET_STATE_HS);
		IRIS_LOGI("mcu_sw_reset: %x", mcu_sw_reset);
		if ((mcu_sw_reset & 0x1) == 0x0) {
			mcu_info_2 = iris_ocp_read(IRIS_MCU_INFO_2, IRIS_CMD_SET_STATE_HS);
...
static bool iris_n2m_enlarge(u32 prev_sw_n2m, u32 curr_sw_n2m)
{
	u32 prev_n, prev_m;
	u32 curr_n, curr_m;
	...
	if (prev_sw_n2m == 0) {
		...
		prev_n = prev_sw_n2m & 0x3ff;
		prev_m = (prev_sw_n2m >> 16) & 0x3ff;
	}
	...
	if (curr_sw_n2m == 0) {
		...
		curr_n = curr_sw_n2m & 0x3ff;
		curr_m = (curr_sw_n2m >> 16) & 0x3ff;
	}

	prev_val = prev_m * curr_n;
	curr_val = curr_m * prev_n;
	ret = curr_val > prev_val;

	IRIS_LOGI("prev_sw_n2m:%x %d:%d, curr_sw_n2m:%x %d:%d, comp %d:%d, ret: %d",
		prev_sw_n2m, prev_n, prev_m, curr_sw_n2m, curr_n, curr_m, prev_val, curr_val, ret);

	return ret;
}

static void iris_dtg_setting_update(void *handle, bool lock, bool i2c_path)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 *payload = NULL;
	u32 dtg_vsw, dtg_vbp, dtg_vres, dtg_vfp, dtg_vfp_min, dtg_vfp_max, dtg_vtotal;
	u32 ivsa_filter;
	u32 te_filter;
	u32 fi_preload_dly;
	...
	//	return;
	IRIS_LOGI("handle: %d, lock: %d, i2c_path: %d", handle != NULL, lock, i2c_path);
...
#else
	if (lock && pcfg->lightup_ops.acquire_panel_lock)
...
void iris_update_panel_ap_te_i7p(void *handle, u32 new_te)
{
	...
	if ((pcfg->rx_mode == IRIS_VIDEO_MODE) && (pcfg->tx_mode == IRIS_VIDEO_MODE)) {
		...
		if (pcfg->abyp_ctrl.abypass_mode != PASS_THROUGH_MODE) {
			pcfg->panel_te = pcfg->ap_te;

			if (new_te != pcfg->panel_te) {
				pcfg->panel_te = new_te;
				pcfg->ap_te = new_te;
				IRIS_LOGI("[%s:%d], abypass new panel TE: %d", __func__, __LINE__, new_te);
			}
			pcfg->sw_n2m = 0;
		} else {
			// PT mode
...
static void iris_dtg_disable_n2m_i7p(void)
{
	u32 cmd[4];
	struct iris_cfg *pcfg = iris_get_cfg();

	if (pcfg->sw_n2m == 0 || pcfg->sw_n2m == 0x40010001)
		return;
	cmd[0] = IRIS_DTG_ADDR + DTG_FRC_SW_CTRL2;
	cmd[1] = 0x0;
	cmd[2] = IRIS_DTG_ADDR + DTG_UPDATE;
	cmd[3] = 0xf;
	iris_ocp_write_mult_vals(4, cmd);

	IRIS_LOGI("%s, old sw_n2m: %d", __func__, pcfg->sw_n2m);
	pcfg->sw_n2m = 0;
}

static void iris_dtg_n2m_mode(bool frc_only)
{
	u32 cmd[4];
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 sw_n2m = pcfg->sw_n2m;
...
void iris_dtg_te_n2m_ctrl_setting_send_i7p(bool enable)
{
	...
	if ((pcfg->rx_mode == IRIS_VIDEO_MODE) && (pcfg->tx_mode == IRIS_VIDEO_MODE)) {
		// set n2m mode in iris_dtg_setting_update when timing switch
		return;
	}
	...
	if (enable && pcfg->memc_info.memc_mode == MEMC_DISABLE) {
		...
		cmd[1] = 0x00000101;
	cmd[0] = IRIS_DTG_ADDR + DTG_TE_N2M_CTRL;
	cmd[2] = IRIS_DTG_ADDR + DTG_UPDATE;
	cmd[3] = 0xe;
	iris_ocp_write_mult_vals(4, cmd);
...
{
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_panel_timing_info timing;

	if (!pcfg || !pcfg->iris_memc_ops.iris_memc_get_main_panel_timing_info) {
		IRIS_LOGE("invalid params!, %s", __func__);
		return;
	}

	if (pcfg->iris_memc_ops.iris_memc_get_main_panel_timing_info(&timing))
		return;

	if (pcfg->pwil_mode != RFB_MODE)
		pcfg->panel_te = timing.fps;
	pcfg->ap_te = timing.fps;
	// temp treat display timing same as input timing
	pcfg->frc_setting.disp_hres = timing.width;
	pcfg->frc_setting.disp_vres = timing.height;
}
...
void iris_parse_memc_param_i7p(void)
{
...
void iris_memc_info_update_i7p(void)
{
	...
	if (pcfg->memc_info.low_latency_mode >= LT_INVALID ||
		(pcfg->memc_info.memc_mode != MEMC_SINGLE_VIDEO_ENABLE &&
		pcfg->memc_info.memc_mode != MEMC_SINGLE_GAME_ENABLE))
...
void iris_frc_setting_update_i7p(void)
{
	...
	if (!(debug_mv_res_overwrite & 0x80000000) && (frc_setting->vid_buf_num == 3) &&
		(pcfg->memc_info.mv_hres * pcfg->memc_info.mv_vres >
			pcfg->frc_setting.init_single_mv_hres * pcfg->frc_setting.init_single_mv_vres)) {
		frc_setting->mv_hres = pcfg->frc_setting.init_single_mv_hres;
		frc_setting->mv_vres = pcfg->frc_setting.init_single_mv_vres;

		IRIS_LOGI("%s(), format mv: [0x%08x] -> [0x%08x].", __func__,
			(pcfg->memc_info.mv_hres << 16) | pcfg->memc_info.mv_vres,
...
void iris_mcu_version_read_i7p(void)
{
	...
	if (!ret) {
		pcfg->app_version1 = iris_ocp_read(IRIS_PROXY_ADDR, IRIS_CMD_SET_STATE_HS);
		ret = true;
	}
}
...
void iris_memc_ctrl_pt_frc_meta_set_i7p(bool frc_enable)
{
	...
	if (frc_enable) {
		/* send pwil meta cmd */
		iris_set_pwil_mode_i7p(FRC_MODE, IRIS_CMD_SET_STATE_HS, true);
		...
		if ((pcfg->rx_mode == IRIS_VIDEO_MODE) && (pcfg->tx_mode == IRIS_VIDEO_MODE)
			...
			mode = RFB_MODE;
		/* send pwil meta cmd */
		iris_set_pwil_mode_i7p(mode, IRIS_CMD_SET_STATE_HS, true);
		pcfg->frc_enabled = false;
	}
...
void iris_memc_ctrl_pt_rfb_meta_set_i7p(bool rfb_enable)
{
	...
	if (rfb_enable) {
		/* send pwil meta cmd */
		iris_set_pwil_mode_i7p(RFB_MODE, IRIS_CMD_SET_STATE_HS, true);
	} else {
		/* send pwil meta cmd */
		iris_set_pwil_mode_i7p(PT_MODE, IRIS_CMD_SET_STATE_HS, true);
		pcfg->memc_info.n2m_mode = 0;
	}
}
...
void iris_dport_output_mode_select_i7p(bool enable)
{
	...
	if (!enable)
		mode = 0;
	else if (pcfg->tx_mode == IRIS_CMD_MODE)
		mode = 2;
	else
		mode = 1;
...
void iris_switch_timeout_dump_i7p(void)
{
	...
	dump->trigger = true;
	dump->sw_pwr_ctrl = iris_ocp_read(0xf0000068, IRIS_CMD_SET_STATE_HS);
	dump->rx_frame_cnt1 = iris_ocp_read(0xf1640354, IRIS_CMD_SET_STATE_HS);
	dump->rx_video_meta = iris_ocp_read(0xf1640080, IRIS_CMD_SET_STATE_HS);
	dump->pwil_cur_meta0 = iris_ocp_read(0xf1600228, IRIS_CMD_SET_STATE_HS);
	dump->pwil_status = iris_ocp_read(0xf160015c, IRIS_CMD_SET_STATE_HS);
	dump->pwil_disp_ctrl0 = iris_ocp_read(0xf1601000, IRIS_CMD_SET_STATE_HS);
	dump->pwil_int = iris_ocp_read(0xf161ffe4, IRIS_CMD_SET_STATE_HS);
	dump->dport_int = iris_ocp_read(0xf119ffe4, IRIS_CMD_SET_STATE_HS);
	if (dump->sw_pwr_ctrl & 0x10) {
		dump->fi_debugbus[0] = iris_ocp_read(0xf219ffd8, IRIS_CMD_SET_STATE_HS);
		dump->fi_debugbus[1] = iris_ocp_read(0xf219ffd8, IRIS_CMD_SET_STATE_HS);
		dump->fi_debugbus[2] = iris_ocp_read(0xf219ffd8, IRIS_CMD_SET_STATE_HS);
	}
...
void iris_memc_ctrl_cmd_proc_i7p(u32 cmd)
{
	...
	case MEMC_CTRL_FRC_PREPARE:
		...
		// support 15fps video
		if (pcfg && pcfg->iris_memc_ops.iris_set_idle_check_interval)
			pcfg->crtc0_old_interval = pcfg->iris_memc_ops.iris_set_idle_check_interval(0, 70);
	...
	case MEMC_CTRL_FRC2PT:
		...
		if (pcfg && pcfg->crtc0_old_interval != 0
	...
	case MEMC_CTRL_RFB2PT:
		// always disable n2m before rfb2pt in video mode
		if ((pcfg->rx_mode == IRIS_VIDEO_MODE) && (pcfg->tx_mode == IRIS_VIDEO_MODE))
			iris_dtg_disable_n2m_i7p();
...
int iris_debug_memc_option_get_i7p(char *kbuf, int size)
{
	...
	len += snprintf(kbuf + len, size - len,
			"%d-%s: %x\n", DEBUG_DSPP_PQ_DISABLE, "dspp_pq_disable", pcfg->iris_pq_disable);
	len += snprintf(kbuf + len, size - len,
...
int iris_dbgfs_memc_init_i7p(void)
{
	struct iris_cfg *pcfg;
	...
	debugfs_create_u32("debug_dtg", 0644, pcfg->dbg_root,
		(u32 *)&iris_debug_dtg);
	debugfs_create_u32("n2m_in_frc_only", 0644, pcfg->dbg_root,
		(u32 *)&iris_debug_n2m_in_frc_only);
	return 0;
}
...
u32 iris_pwil_mode_state_get_i7p(void)
{
	...
	rc = iris_ocp_read(IRIS_PWIL_ADDR + PWIL_STATUS, IRIS_CMD_SET_STATE_HS);
...
void iris_kernel_status_get_i7p(u32 get_op, u32 count, u32 *values)
{
	...
	case GET_IRIS_PWIL_MODE_STATE:
		if (!pcfg || !pcfg->lightup_ops.acquire_panel_lock || !pcfg->lightup_ops.release_panel_lock)
			break;
		pcfg->lightup_ops.acquire_panel_lock();
	...
	case GET_AP_DISP_MODE_STATE:
		if (pcfg->iris_is_curmode_cmd_mode() &&
			pcfg->rx_mode == IRIS_CMD_MODE)
			*values = 1;
		else if (pcfg->iris_is_curmode_vid_mode() &&
			pcfg->rx_mode == IRIS_VIDEO_MODE)
			*values = 2;
		else
			*values = 0;
...
void iris_debug_info_get_i7p(u32 *value, u32 count)
{
	...
	switch (value[0]) {
	case 0:
		if (pcfg->lightup_ops.get_display_info)
			len = pcfg->lightup_ops.get_display_info(buf, count);
		break;
	case 1:
...
void iris_lightoff_memc_i7p(void)
{
	...
	iris_set_pwil_mode_i7p(PT_MODE, IRIS_CMD_SET_STATE_HS, false);
}
...
void iris_memc_func_init_i7p(struct iris_memc_func *memc_func)
{
	...
	memc_func->parse_memc_param = iris_parse_memc_param_i7p;
	...
	memc_func->iris_demo_wnd_set = iris_demo_wnd_set_i7p;
}
