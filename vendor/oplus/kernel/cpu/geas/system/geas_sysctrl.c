...
int geas_frame_arrive_handler(const struct ctl_table *table, int write,
...
int geas_frame_drive_handler(const struct ctl_table *table, int write,
			void *buffer, unsigned long *lenp, long long *ppos);
...
int geas_create_sysctrl_for_node(const char *dev_name, struct hwmon_node_ext *bwmon_ext)
{
	...
	register_sysctl_sz(sysctrl_dir, bwmon_ext->geas_table, sizeof(geas_table) / sizeof(geas_table[0]));
	kfree(sysctrl_dir);
