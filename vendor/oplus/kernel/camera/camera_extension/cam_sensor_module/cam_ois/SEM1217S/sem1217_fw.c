...
int SEM1217S_Store_OIS_Cal_Data (void)
{
	...
	CAM_EXT_INFO(CAM_EXT_OIS, "SEM1217S_REG_OIS_STS: %u", rxdata[0]);
	...
	CAM_EXT_INFO(CAM_EXT_OIS, "write SEM1217S_REGINFO_BLK_UP_CTRL: %u", txdata[0]);
	...
	do
	{
		if (repeatedCnt == 0)
		{
			...
			CAM_EXT_ERR(CAM_EXT_OIS, "REG_INFO_BLK_UP_CTRL failed: %u", rxdata[0]);
			return 0;
		}
	...
	if ((data_error & SEM1217S_ERR_ODI) != SEM1217S_NO_ERROR)
	{
		...
		CAM_EXT_ERR(CAM_EXT_OIS, "SEM1217S_ERR_ODI error %d", data_error);
		return 0;
	}
...
struct SEM1217S_FACT_ADJ SEM1217S_Gyro_offset_cal(void)
{
	...
	rxdata[0] = sem1217_8bit_read(SEM1217S_REG_OIS_STS);
	CAM_EXT_ERR(CAM_EXT_OIS, "SEM1217S_REG_OIS_STS: %u", rxdata[0]);
	...
	do
	{
		...
		CAM_EXT_INFO(CAM_EXT_OIS, "SEM1217S_REG_GCAL_CTRL: %u", rxdata[0]);
		repeatedCnt--;
	...
	{
		CAM_EXT_ERR(CAM_EXT_OIS, "SEM1217S offset cal failed, data_error: 0x%x", data_error);
	...
	CAM_EXT_INFO(CAM_EXT_OIS, "sem1217s x-offset: %u, y-offset: %u", SEM1217S_FADJ_CAL.gl_GX_OFS, SEM1217S_FADJ_CAL.gl_GY_OFS);
...
void GyroRead(uint32_t address)
{
	...
	CAM_EXT_INFO(CAM_EXT_OIS, "[GyroRead] address= 0x%x, read = 0x%x %x %x %x", address, rxdata[0], rxdata[1], rxdata[2], rxdata[3]);
}
...
void GyroWrite(uint32_t address, uint32_t gain)
{
	...
	CAM_EXT_INFO(CAM_EXT_OIS, "[GyroRead] gain = %u, address= 0x%x, write = 0x%x %x %x %x", gain, address, txdata[0], txdata[1], txdata[2], txdata[3]);
}
...
void SEM1217S_Gyro_gain_set(uint32_t X_gain, uint32_t Y_gain)
{
	uint8_t rxdata[SEM1217S_RX_BUFFER_SIZE];
	rxdata[0] = sem1217_8bit_read(SEM1217S_REG_OIS_STS);
	CAM_EXT_INFO(CAM_EXT_OIS, "SEM1217S_Gyro_gain_set SEM1217S_REG_OIS_STS: %u", rxdata[0]);
	...
	{
		CAM_EXT_INFO(CAM_EXT_OIS, "[SEM1217S_Gyro_gain_set] use default gyro gain, when X_gain= %u  Y_gain= %u", X_gain, Y_gain);
	}
	else
	{
		CAM_EXT_INFO(CAM_EXT_OIS, "[SEM1217S_Gyro_gain_set] newGyorGain  X_gain= %u  Y_gain= %u",X_gain, Y_gain);
		/* Set GYRO_GAIN_X */
...
{
	int rc = 0;
	CAM_EXT_INFO(CAM_EXT_OIS, "[SEM1217S_WriteGyroGainToFlash]");
	rc = SEM1217S_Store_OIS_Cal_Data();
	GyroRead(SEM1217S_REG_GX_GAIN); /* Read gyro gain x */
	GyroRead(SEM1217S_REG_GY_GAIN); /* Read gyro gain y */
}
...
int sem1217s_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_EXT_ERR(CAM_EXT_OIS, "current_fw_ver: 0x%x, new_fw_ver: 0x%x", current_fw_ver, new_fw_ver);
	if (current_fw_ver == new_fw_ver)
	{
		CAM_EXT_ERR(CAM_EXT_OIS, "version is the same, no need to update");
		return 0;
	}

	if (o_ctrl->actuator_ois_eeprom_merge_flag)
	{
		CAM_EXT_DBG(CAM_EXT_OIS, "before actuator_ois_eeprom_merge_flag lock");
		mutex_lock(o_ctrl->actuator_ois_eeprom_merge_mutex);
		CAM_EXT_DBG(CAM_EXT_OIS, "after actuator_ois_eeprom_merge_flag lock");
	}
	...
	for (i = 0; i < (SEM1217S_APP_FW_SIZE / txBuffSize); i++)
	{
		...
		CAM_EXT_ERR(CAM_EXT_OIS, "update ois fw blk_num: %d", i+1);
		idx += txBuffSize;
		mdelay(20);
	}
	...
	{
		CAM_EXT_ERR(CAM_EXT_OIS, "update fw erro");
		return -1;
	}
	...
	CAM_EXT_ERR(CAM_EXT_OIS, "updated_ver: 0x%x, new_fw_ver: 0x%x", updated_ver, new_fw_ver);
	if (updated_ver != new_fw_ver)
	{
		CAM_EXT_ERR(CAM_EXT_OIS, "update fw failed, update version is not equal with read");
		return -1;
	}
error_hand:
	mutex_unlock(o_ctrl->actuator_ois_eeprom_merge_mutex);

	CAM_EXT_ERR(CAM_EXT_OIS, "update fw end, rc: %d", rc);
