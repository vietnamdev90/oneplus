	...
	*/
	...
	for (i = 0; i < LOOP_A; i++) {
		...
		if (r_data == ref) {
			...
			if (i >= LOOP_B) {
				CAM_EXT_ERR(CAM_EXT_OIS,"[wait_check_register] fail: 0x%04X", r_data);
...
int dw9786_id_check(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	if (chip_id != DW9786_CHIP_ID) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_id_check] the module's OIS IC is not dw9786 (0x%04X)", chip_id);
		return -1;
	}
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_id_check] the module's OIS IC is dw9786 (0x%04X)", chip_id);
	return 0;
}
...
int dw9786_auto_read_check(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	mdelay(5);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] start....");
	...
	if (autord_rv_error != AUTORD_RV_OK) {
		...
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 auto_read fail (0x%04X)", autord_rv_error);
		if ((autord_rv_error & 0xFFFF) == 0x0001)
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 LUT_ERR");
		if ((autord_rv_error & 0xFFFF) == 0x0002)
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 PGM_ERR");
		if ((autord_rv_error & 0xFFFF) == 0x0004)
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 LDT0_ERR");
		if ((autord_rv_error & 0xFFFF) == 0x0008)
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 RED_ERR");
		if ((autord_rv_error & 0xFFFF) == 0x0010)
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 DCT_ERR");
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_auto_read_check] dw9786 auto-lead function passed");
	}
...
int dw9786_download_fw(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	if(rc != FUNC_PASS){
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] reset failed...");
		return rc;
	}
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] read actuator_id %d",ac_id);
	if(ac_id == 1){
		...
		date = DW9786_FW_DATE_DVT;
		checksum = DW9786_REF_MCS_CHECKSUM_DVT;
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] read actuator_id failed %d, force to download fw...",ac_id);
	}
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] module prj_info:[0x%.4x] ver:[0x%.4x] date:[0x%.4x]", pjt, ver, dat);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] fw prj_info:[0x%.4x] ver:[0x%.4x] date:[0x%.4x]", project, version, date);
	if ((dat != date) || (g_downloadByForce)) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] the new firmware version is checked and the update starts.");
		if (dw9786_code_update(o_ctrl,ac_id,checksum) != FUNC_PASS) {
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw]return fail");
			return FUNC_FAIL;
		}
		else if (dw9786_checksum_read(o_ctrl) != checksum) {
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] start download firmware due to a checksum error.");
		}
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw] this is the latest version.");
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_download_fw]return pass");
	return FUNC_PASS;
}
...
int dw9786_code_update(struct cam_ois_ctrl_t *o_ctrl,int ac_id, uint32_t checksum)
{
	...
	if(ac_id == 1){
		...
		DW9786_FW_ARRAY = DW9786_FW_DVT;
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update]ac_id inputed error %d !!! Force to download...",ac_id);
	}
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] start....");
	/*MCS start -----------------------------------------------------------------------------*/
	...
	for (i = 0; i < FMC_PAGE; i++) {
		...
		if(rc < 0){
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] Set erase address %d failed, and then exit",addr);
			break;
		}
		addr += 0x800;
		mdelay(5);
	...
	for (i = 0; i < MCS_SIZE; i += MCS_PKT_SIZE) {
		...
		if(rc < 0){
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] Write DW9786_FW[%d] failed, and then exit",i);
			return rc;
		}
	...
	if (checksum_criteria != checksum_flash) {
		rc = CHECKSUM_ERROR;
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] flash checksum failed!, checksum_criteria: 0x%08X, checksum_flash: 0x%08X", checksum_criteria, checksum_flash);
		dw9786_chip_enable(o_ctrl, MODE_OFF);
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] firmware download failed.");
		return rc;
	}
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] flash checksum passed!, checksum_criteria: 0x%08X, checksum_flash: 0x%08X", checksum_criteria, checksum_flash);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_code_update] firmware download successful.");
	return rc;
}
...
int dw9786_checksum_func(struct cam_ois_ctrl_t *o_ctrl, int type)
{
	...
	*/
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_checksum_func] flash memory checksum , [0x%08X]", mcs_checksum_flash);
...
int dw9786_checksum_read(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_checksum_read] flash memory checksum , [0x%08X]", checksum_register);
...
struct _FACT_ADJ_DW dw9786_gyro_ofs_calibration(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] start....");
	...
	if (wait_check_register(o_ctrl, 0xB020, 0x6000) == FUNC_PASS) {
		...
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] gyro offset EXECUTE_ERROR");
		return FADJCAL;
	}


	if (wait_check_register(o_ctrl, 0xB020, 0x6001) == FUNC_PASS) { // when calibration is done, Status changes to 0x6001
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] calibration function finish");
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] calibration function error");
		return FADJCAL;
	}
	...
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] x gyro offset: 0x%04X(%d)", x_ofs, (short)x_ofs);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] y gyro offset: 0x%04X(%d)", y_ofs, (short)y_ofs);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] gyro_status: 0x%04X", gyro_status);

	if((gyro_status & 0x8000) == 0x8000) { // Read Gyro offset cailbration result status
		if (gyro_status & X_AXIS_GYRO_OFS_CAL_PASS) {
			//msg += OK;
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] x_axis gyro offset calibration pass");
		}
		if (gyro_status & X_AXIS_GYRO_OFS_MAX_ERR) {
			//msg += X_AXIS_GYRO_OFS_MAX_ERR;
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] x_axis gyro offset max. error");
		}
		if (gyro_status & Y_AXIS_GYRO_OFS_CAL_PASS) {
			//msg += OK;
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] y_axis gyro offset calibration pass");
		}
		if (gyro_status & Y_AXIS_GYRO_OFS_MAX_ERR) {
			//msg += Y_AXIS_GYRO_OFS_MAX_ERR;
			CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] y_axis gyro offset max. error");
		}
	}
	else {
		//msg += GYRO_OFS_CAL_DONE_FAIL;
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_gyro_ofs_calibration] calibration done fail");
	}
...
{
	uint32_t r_data;
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] start....");

	read_reg_16bit_value_16bit(o_ctrl, 0xE018, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] product_id: 0x%04X", r_data);
	read_reg_16bit_value_16bit(o_ctrl, 0xB000, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] chip_id: 0x%04X", r_data);
	read_reg_16bit_value_16bit(o_ctrl, 0xB002, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] Fw_ver_H: 0x%04X", r_data);
	read_reg_16bit_value_16bit(o_ctrl, 0xB004, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] Fw_ver_L: 0x%04X", r_data);
	read_reg_16bit_value_16bit(o_ctrl, 0xB006, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_info] Fw_date: 0x%04X", r_data);
}

void dw9786_fw_info(struct cam_ois_ctrl_t *o_ctrl)
{
	uint32_t r_data;
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_fw_info] start....");

	read_reg_16bit_value_16bit(o_ctrl, 0x9800, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_fw_info] set version: 0x%02X [MSB], project no.:0x%02X [LSB]", r_data >> 8, r_data & 0xff);
	read_reg_16bit_value_16bit(o_ctrl, 0x9802, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_fw_info] fw version: 0x%02X [MSB], pid version:0x%02X [LSB]", r_data >> 8, r_data & 0xff);
	read_reg_16bit_value_16bit(o_ctrl, 0x9804, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_fw_info] fw_date: 0x%04X", r_data);
	read_reg_16bit_value_16bit(o_ctrl, 0x9806, &r_data);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_fw_info] act: 0x%02X [MSB], module:0x%02X [LSB]", r_data >> 8, r_data & 0xff);
}
...
int dw9786_module_store(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	-1 : EXECUTE_ERROR
	*/
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_module_store] start....");
	...
	if (wait_check_register(o_ctrl, 0xB020, 0xA000) == FUNC_PASS) {
		...
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_module_store] execute fail");
		return EXECUTE_ERROR;
	}

	if (wait_check_register(o_ctrl, 0xB020, 0xA001) == FUNC_PASS) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_module_store] complete");
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_module_store] fail");
		return EXECUTE_ERROR;
	}
...
int dw9786_set_store(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	-1 : EXECUTE_ERROR
	*/
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_set_store] start....");
	write_reg_16bit_value_8bit(o_ctrl, 0x97E6, 0xEA); //USER_PT_C off
	write_reg_16bit_value_8bit(o_ctrl, 0x3FFF, 0x00); //User memory dummy write
	mdelay(10); //Absolutely necessary
	...
	if (wait_check_register(o_ctrl, 0xB020, 0xA000) == FUNC_PASS) {
		...
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_set_store] execute fail");
		return EXECUTE_ERROR;
	}

	if (wait_check_register(o_ctrl, 0xB020, 0xA001) == FUNC_PASS) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_set_store] complete");
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_set_store] fail");
		return EXECUTE_ERROR;
	}
...
int dw9786_module_erase(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	*/
	...
	//uint32_t r_data;
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_erase] start....");
	...
	if (wait_check_register(o_ctrl, 0xB020, 0xB000) == FUNC_PASS) {	//Read Status
		...
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_erase] fail");
		return EXECUTE_ERROR;
	}

	if (wait_check_register(o_ctrl, 0xB020, 0xB001) == FUNC_PASS) {	//Read Status
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_erase] complete");
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_erase] fail");
...
{
	int rc = FUNC_PASS;
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_reset] start....");

	rc = write_reg_16bit_value_16bit(o_ctrl, 0xE000, 0x0000); //shutdown mode
	if(rc != FUNC_PASS){
	    CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_reset] write 0xE000 fail, exit....");
        return rc;
    }
	mdelay(10);
	...
	mdelay(100);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_reset] reset execution complete");
	return rc;
}
...
{
	if (mode == 0) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_mcu_active] dw9786_standby");
		write_reg_16bit_value_16bit(o_ctrl, 0xE004, mode);
	}
	else {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_mcu_active] dw9786_idle mode");
		write_reg_16bit_value_16bit(o_ctrl, 0xE004, mode);
	}
	return FUNC_PASS;
}
...
{
	if (en == 0) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_enable] dw9786_sleep");
		write_reg_16bit_value_16bit(o_ctrl, 0xE000, en);
	}
	else if(en == 1) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_chip_enable] dw9786_standby");
		write_reg_16bit_value_16bit(o_ctrl, 0xE000, en);
	}
	return FUNC_PASS;
}
...
{
	write_reg_16bit_value_16bit(o_ctrl, 0xEDB0, 0xA23F);
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_user_protection] excute");
	return FUNC_PASS;
}
...
int dw9786_servo_on(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if(wait_check_register(o_ctrl, 0xB020, 0x1022) == FUNC_PASS) { //busy check
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_servo_on] servo on success");
		return FUNC_PASS;
	}
...
int dw9786_servo_off(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if(wait_check_register(o_ctrl, 0xB020, 0x1000) == FUNC_PASS) {	//busy check
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_servo_off] servo off success");
		return FUNC_PASS;
	}
...
int dw9786_ois_on(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if(wait_check_register(o_ctrl, 0xB010, 0x1021) == FUNC_PASS) {
		CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_ois_on] ois on success");
		return FUNC_PASS;
	}
...
{
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_spi_master_mode] set to gyro spi master mode.");
...
{
	CAM_EXT_ERR(CAM_EXT_OIS,"[dw9786_spi_intercept_mode] set to gyro spi intercept mode.");
...
void DW9786_WriteGyroGainToFlash(struct cam_ois_ctrl_t *o_ctrl,uint32_t X_gain, uint32_t Y_gain)
{
    ...
    CAM_EXT_INFO(CAM_EXT_OIS, "[DW9786_WriteGyroGainToFlash] oldGyorGain  X_gain= 0x%x  Y_gain= 0x%x",X_ori, Y_ori);
    CAM_EXT_INFO(CAM_EXT_OIS, "[DW9786_WriteGyroGainToFlash] newGyorGain  X_gain= 0x%x  Y_gain= 0x%x",X_gain, Y_gain);
    write_reg_16bit_value_16bit(o_ctrl,dw_Gyro_gain_x, X_gain);
    write_reg_16bit_value_16bit(o_ctrl,dw_Gyro_gain_y, Y_gain);
    //dw9786_set_store(o_ctrl);
    read_reg_16bit_value_16bit(o_ctrl, dw_Gyro_gain_x, &X_ori);//read 0xB806
    read_reg_16bit_value_16bit(o_ctrl, dw_Gyro_gain_y, &Y_ori);//read 0xB808
    CAM_EXT_INFO(CAM_EXT_OIS, "[DW9786_WriteGyroGainToFlash] after write  X_gain= 0x%x  Y_gain= 0x%x",X_ori, Y_ori);
...
void DW9786_StoreGyroGainToFlash(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    read_reg_16bit_value_16bit(o_ctrl, dw_Gyro_gain_x, &X_ori);//read 0xB806
    read_reg_16bit_value_16bit(o_ctrl, dw_Gyro_gain_y, &Y_ori);//read 0xB808
    CAM_EXT_INFO(CAM_EXT_OIS, "[DW9786_WriteGyroGainToFlash] store GyorGain  X_gain= 0x%x  Y_gain= 0x%x",X_ori, Y_ori);
    dw9786_set_store(o_ctrl);
}
