...
static struct clk_vdd_class *gcc_canoe_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_canoe_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_canoe_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static struct clk_regmap *gcc_canoe_clocks[] = {
...
static const struct qcom_reset_map gcc_canoe_resets[] = {
...
static const struct qcom_cc_desc gcc_canoe_desc = {
...
static const struct of_device_id gcc_canoe_match_table[] = {
	{ .compatible = "qcom,canoe-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_canoe_match_table);

static int gcc_canoe_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_canoe_desc);
	if (IS_ERR(regmap))
...
static void gcc_canoe_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_canoe_desc);
}

static struct platform_driver gcc_canoe_driver = {
	.probe = gcc_canoe_probe,
	.driver = {
		.name = "qcom,canoe-gcc",
		.of_match_table = gcc_canoe_match_table,
		.sync_state = gcc_canoe_sync_state,
	},
};

static int __init gcc_canoe_init(void)
{
	return platform_driver_register(&gcc_canoe_driver);
}
subsys_initcall(gcc_canoe_init);

static void __exit gcc_canoe_exit(void)
{
	platform_driver_unregister(&gcc_canoe_driver);
}
module_exit(gcc_canoe_exit);

MODULE_DESCRIPTION("QTI GCC CANOE Driver");
MODULE_LICENSE("GPL");
