...
static struct clk_hw *debugcc_sc7280_hws[] = {
...
static const struct of_device_id clk_debug_match_table[] = {
	{ .compatible = "qcom,debugcc-sc7280" },
	{ }
};

static int clk_debug_sc7280_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_sc7280_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_sc7280_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				qcom_clk_hw_get_name(debugcc_sc7280_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_sc7280_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_sc7280_init);

MODULE_DESCRIPTION("QTI DEBUG CC SC7280 Driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:debugcc-sc7280");
