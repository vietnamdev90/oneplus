...
static int gcc_debug_mux_sels[] = {
	...
	0x1F1,		/* gcc_camera_hf_axi_cloned_clk */
	...
	0x21A,		/* gcc_pcie_0_pipe_div2_clk */
	...
	0x21B,		/* gcc_qupv3_wrap2_s5_clk */
	0x21C,		/* gcc_qupv3_wrap2_s6_clk */
	0x21D,		/* gcc_qupv3_wrap2_s7_clk */
...
static int gpu_cc_debug_mux_sels[] = {
	...
	0x2A,		/* measure_only_gpu_cc_cb_clk */
	0xE,		/* measure_only_gpu_cc_cxo_aon_clk */
	0x2B,		/* measure_only_gpu_cc_rscc_hub_aon_clk */
...
static struct clk_hw *debugcc_alor_hws[] = {
...
static const struct of_device_id clk_debug_match_table[] = {
	{ .compatible = "qcom,alor-debugcc" },
	{ }
};

static int clk_debug_alor_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_alor_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_alor_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				qcom_clk_hw_get_name(debugcc_alor_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_alor_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_alor_init);

MODULE_DESCRIPTION("QTI DEBUG CC ALOR Driver");
MODULE_LICENSE("GPL");
