...
static int virtio_add_region(const struct md_region *entry)
{
	...
	unsigned int len;
	int ret = 0;
//	if (!pfn_is_map_memory((unsigned long)entry->virt_addr)) {
	if (!pfn_is_map_memory(virt_to_pfn((void *)entry->virt_addr))) {
...
static int virtio_md_init_vqs(struct virtio_minidump *vmd)
{
	...
	err = virtio_find_vqs(vmd->vdev, 1, vqs, vqs_info, NULL);
	if (err)
		return err;
	vmd->vq = vqs[0];
