...
int stub_physical_channel_send(struct physical_channel *pchan,
			struct hab_header *header,
...
int stub_physical_channel_read(struct physical_channel *pchan,
				void *payload,
				size_t read_size)
{
	struct loopback_dev *dev = pchan->hyp_data;
...
void stub_physical_channel_rx_dispatch(unsigned long data)
{
}

int stub_habhyp_commdev_alloc(void **commdev, int is_be, char *name,
...
int stub_habhyp_commdev_dealloc(void *commdev)
{
