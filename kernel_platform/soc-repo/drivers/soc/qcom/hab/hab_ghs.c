...
int ghs_habhyp_commdev_alloc(void **commdev, int is_be, char *name, int vmid_remote,
		struct hab_device *mmid_device)
{
	...
	*ppchan = pchan;
	...
	ret = ghs_habhyp_commdev_create_dispatcher(pchan);
	if (ret)
		goto err;
...
int ghs_habhyp_commdev_dealloc(void *commdev)
{
	...
	ghs_habhyp_commdev_dealloc_os(commdev);
...
void ghs_hab_hypervisor_unregister(void)
{
	pr_debug("total %d\n", hab_driver.ndevices);
...
int ghs_hab_hypervisor_register(void)
{
	int ret = 0;

	/* os-specific registration work */
	ret = ghs_hab_hypervisor_register_os();

	return ret;
}

int ghs_hab_hypervisor_register_post(void) { return 0; }
