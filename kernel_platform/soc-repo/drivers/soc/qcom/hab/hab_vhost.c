...
int vhost_hab_hypervisor_register(void)
{
...
void vhost_hab_hypervisor_unregister(void)
{
	uint32_t max_devices = HABCFG_MMID_AREA_MAX + 1;
...
int vhost_hab_hypervisor_register_post(void) { return 0; }

void vhost_hab_pipe_read_dump(struct physical_channel *pchan) {};

void vhost_dump_hab_wq(struct physical_channel *pchan) {};
...
int vhost_physical_channel_send(struct physical_channel *pchan,
			struct hab_header *header,
			void *payload,
			unsigned int flags)
{
...
int vhost_physical_channel_read(struct physical_channel *pchan,
						void *payload,
						size_t read_size)
{
	struct vhost_hab_pchannel *vh_pchan = pchan->hyp_data;
...
void vhost_physical_channel_rx_dispatch(unsigned long physical_channel)
{
	struct physical_channel *pchan =
...
int vhost_habhyp_commdev_alloc(void **commdev, int is_be, char *name,
...
int vhost_habhyp_commdev_dealloc(void *commdev)
{
...
int vhost_hab_stat_log(struct physical_channel **pchans, int pchan_cnt, char *dest,
			int dest_size)
{
